...
function handleUnlimitedTopNavRender() {
    ...
    function handleMenuButtonAction(element, direction) {
        ...
        const containerWidth = document.querySelector('.app-top-nav').clientWidth - document.querySelector('.app-top-nav').clientHeight * 2;
        let totalWidth = 0;
    ...
    function handlePageLoadMenuFocus() {
        ...
        const elms = [].slice.call(document.querySelectorAll('.app-top-nav .menu > .menu-item'));
        if (elms) {
            let found = false;
            elms.map(function (elm) {
                if (!elm.classList.contains('menu-control')) {
