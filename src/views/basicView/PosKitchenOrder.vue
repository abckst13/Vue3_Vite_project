        ...
        <card-body class="pos-container">
            ...
            <div class="pos-content">
                <perfect-scrollbar class="pos-content-container h-100 p-0">
                        ...
                        <div class="pos-task-info">
                            ...
                            <div class="mb-2">
                                <span
                                    class="badge fs-14px"
                                    v-bind:class="{
                                        'bg-theme text-black': order.orderStatus != 'Completed',
                                        'bg-gray-500 text-white': order.orderStatus == 'Completed'
                                    }"
                                    >{{ order.orderType }}</span
                                >
                            </div>
                            <div v-if="order.orderTime">
                                <span v-bind:class="{ 'text-danger fw-bold': order.urgent }">{{ order.orderTime }}</span> time
                            </div>
                            <div v-if="order.totalOrderTime">All dish served<br />{{ order.totalOrderTime }} total time</div>
                        </div>
                        <div class="pos-task-body">
                            <div class="fs-16px mb-3">Completed: ({{ getTotalCompletedItems(order.items) }}/{{ order.items.length }})</div>
                            <div class="row gx-4">
                                <div class="col-lg-3 pb-4" v-for="item in order.items">
                                    <div
                                        class="pos-task-product"
                                        v-bind:class="{ completed: item.status == 'Completed' || item.status == 'Cancelled' }"
                                    >
                    ...
                    <div class="px-3 py-5 text-center" v-else>No order found</div>
                </perfect-scrollbar>
            </div>
            <!-- END pos-content -->
        </card-body>
